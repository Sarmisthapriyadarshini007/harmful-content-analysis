document.addEventListener('DOMContentLoaded', () => {
    ...
    if (heatmapGrid) {
        ...
        // Time blocks (Every 4 hours for simplicity)
        const times = ['12AM', '4AM', '8AM', '12PM', '4PM', '8PM'];
        
        times.forEach(time => {
            heatmapGrid.innerHTML += `<div class="heatmap-label">${time}</div>`;
            for (let i = 0; i < 7; i++) {
                // Randomize intensity levels 1 to 4
                // Higher chances of level 3/4 on weekends (index 5,6) and evenings
                let level = Math.floor(Math.random() * 2) + 1; // 1 or 2 default
